SYSTEM_TEMPLATE = """You are a data generator which generates narratives for training language models according to user's need."""
USER_TEMPLATE1 = """The narratives below are one or more example narratives of which each contains 2 paragraphs as 2 segments: a law enforcement report and a coroner/medical examiner report of a youth suicide victim.
------------
"""
...
Now write a new narrative in the same style and format.
You should only output the new story with the 2 paragraphs of a youth suicide victim. Do not output titles or anything other than the narrative."""
...
if __name__ == '__main__':
    ...
    class CFG:
        ...
        GRADIENT_ACCUM = 32
        DROPOUT = 0.0
        MODEL_NAME = 'assets/gemma2_9b_it'
        SEED = 5514
        #P_MAX_LENGTH = 192
        #R_MAX_LENGTH = 384
        NUM_WARMUP_RATE = 0.1
    ...
    # NOTE - change
    data_df = pd.read_csv('data/train_features.csv').iloc[:20]
    label_df = pd.read_csv('data/train_labels.csv').iloc[:20]
    ...
    class train_dataset(Dataset):
        ...
        def __getitem__(self, idx):
            messages = [{"role": "user", "content": SYSTEM_TEMPLATE + " " + USER_TEMPLATE1}
            ...
            for i, idx in enumerate(idxs):
                ...
                cache = f'EXAMPLE{i+1}:\n{le}\n{cme}\n\n\n'
                # NOTE - change
                if i>0 and len((messages[0]['content']+cache).split())>2000:
                # if i>0 and len((messages[1]['content']+cache).split())>2000:
                    break
                # NOTE - change
                messages[0]['content'] = messages[0]['content'] + cache
                # messages[1]['content'] = messages[1]['content'] + cache
            # NOTE - change
            messages[0]['content'] = messages[0]['content'] + USER_TEMPLATE2
    ...
    for step, (input_ids, attention_mask) in its:
        ...
        report = generated_text.split(USER_TEMPLATE2[-30:])[-1]
        ...
        if len(Narratives) >= len(TRAIN_DATASET):
            break
        
    output_df = pd.DataFrame({'Narrative': Narratives})
    output_df.to_csv(f'pure_gendata_{args.id}.csv', index=False)