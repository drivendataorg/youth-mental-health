...
subprocess.run(
    f"pip install --break-system-packages --no-index --find-links=file://{pkg_dir} lightgbm protobuf".split()
)
...
#FEATURES_PATH = Path("data/train_features.csv")
#SUBMISSION_FORMAT_PATH = Path("data/train_labels.csv")
...
def get_llm_preds():
    train = pd.read_csv(MODEL_DIR / "train_features.csv", index_col="uid")

    labels = pd.read_csv(MODEL_DIR / "train_labels.csv", index_col="uid")
    for c in LABEL_COLS[:-2]:
...
def main():
    ...
    logging.info(f"Reading test from {data_dir}..")
    ...
    #test = pd.read_csv(data_path / "train_features.csv", index_col="uid")
    #submission = pd.read_csv(data_path / "train_labels.csv", index_col="uid")

    models_dir = Path("./assets")
    tfidf_word_vec = joblib.load(models_dir / "lgb" / "tfidf_word_vec.job")
    tfidf_char_vec = joblib.load(models_dir / "lgb" / "tfidf_char_vec.job")

    logging.info(f"Transforming test {len(test)}..")
    ...
    models = joblib.load(models_dir / "lgb" / "models.job")
    models[-1] = joblib.load(models_dir / "sgd" / "models.job")[-1]

    is_SuicideIntentDisclosed_seen = False

    y_cross_test_hat_raw = []
    #for (clf, thresh), cols in zip(models, BIN_COLS + CAT_COLS):
    for cols, clf, thresh in models:
        bin_label = cols in BIN_COLS
        logging.info(f"Predicting on full test {X_test.shape}..")
        #is_SuicideIntentDisclosed_seen = False

        if cols in ["WeaponType1"]:
            tfidf_word_vec = joblib.load(models_dir / "sgd" / "tfidf_word_vec.job")
            tfidf_char_vec = joblib.load(models_dir / "sgd" / "tfidf_char_vec.job")
            X_test = sps.hstack(
                [
                    tfidf_word_vec.transform(test["corpus"]),
                    tfidf_char_vec.transform(test["corpus"]),
        ...
        _X_test = (
            #X_test,
            #X_test if not bin_label or len(y_cross_test_hat_raw) == 0 else sps.hstack(
            #X_test if cols == "SuicideIntentDisclosed" or len(y_cross_test_hat_raw) == 0 else sps.hstack(
            X_test if not is_SuicideIntentDisclosed_seen or not bin_label else sps.hstack(
                [
                    X_test,
                    #np.stack(y_cross_test_hat_raw, axis=1),
                    y_cross_test_hat_raw[0][:, None],
                ],
                format="csr",
            )
        )
        ...
        if bin_label:
            ...
            if cols == "SuicideIntentDisclosed":
        ...
        logging.info(f"Making submission for {cols=}..")
