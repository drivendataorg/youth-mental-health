...
def main():
    ...
    train = pd.read_csv(data_dir / "train_features.csv", index_col="uid")
    ...
    #llm_preds = train[LABEL_COLS].copy()
    #llm_preds[LABEL_COLS] = joblib.load("./sub_sgdl1/assets/cv.job")["preds_raw"]
    ...
    llm_preds_train = llm_preds[llm_preds.index.isin(train.index)].copy()
    ...
    for cols in BIN_COLS + CAT_COLS:
        ...
        #if False and (not bin_label or cols in ["DisclosedToOtherFamilyMember"]):
