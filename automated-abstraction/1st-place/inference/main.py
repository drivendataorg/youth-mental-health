...
def inference(args):
    ...
    for model_name in sorted(model_dir.iterdir()):
        ...
        #predictions_bin = (predictions_bin > threshes).astype("int64")
        predictions_b.append(predictions_bin)
        predictions_c1.append(predictions_1)
        predictions_c2.append(predictions_2)

    all_threshes = np.mean(all_threshes, axis=0)#.round().astype("int64")
    predictions_b = np.mean(predictions_b, axis=0)#.round().astype("int64")
